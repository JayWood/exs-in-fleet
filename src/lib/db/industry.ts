...
export async function getMaterialsInput({
  typeID,
  activitySlug
}: getMaterialsInputProps): Promise<IndustryActivityMaterialDocument[]> {
  const activityID =
    ACTIVITIES.find(a => a.slug === activitySlug)?.activityID ?? 0
