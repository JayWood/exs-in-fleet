...
export function calculateStats(orders: MarketOrder[]): OrderStats {
  ...
  const weightedAverage =
    orders.reduce((sum, o) => sum + o.price * o.volume_remain, 0) / totalVolume

  const mean = orders.reduce((sum, o) => sum + o.price, 0) / orders.length
  const variance =
    orders.reduce((sum, o) => sum + Math.pow(o.price - mean, 2), 0) /
    orders.length
  const stddev = Math.sqrt(variance)

  const percentile95 =
    prices[Math.floor(prices.length * 0.95)] || prices[prices.length - 1]
...
export function marketCacheGet(
  filter: Record<string, any> = {},
  fields?: string[]
): Promise<MarketCacheDocument[]> {
